...
const UI = (function() {
    ...
    // ===================================================================
    // ▼▼▼ CENTRAL DE AUDIO: GESTIONA LA COLA Y EVITA CHOQUES ▼▼▼
    // ===================================================================
    const AudioPlayer = (function() {
        let currentAudio = null;
        // La cola ahora almacena objetos { path: string, onEnded: function | null }
        let queue = []; 
        let isPlaying = false;
        ...
        function playNext() {
            ...
            isPlaying = true;
            const item = queue.shift(); // Saca el objeto de la cola
            ...
            currentAudio.onended = () => {
                ...
                // 1. Ejecuta el callback asociado a este audio al finalizar
                if (item.onEnded && typeof item.onEnded === 'function') {
                    item.onEnded();
                }

                // 2. Continúa con el siguiente en la cola
                playNext(); 
            };

            currentAudio.play().catch(error => {
                console.warn(`Error al reproducir ${path}: ${error.message}. Pasando al siguiente en la cola.`);
                currentAudio = null;
                // Si falla, aún ejecuta el callback y pasa al siguiente
                if (item.onEnded && typeof item.onEnded === 'function') {
                    item.onEnded();
                }
                playNext(); 
        ...
        // Función principal para agregar un audio a la cola (AHORA ACEPTA UN CALLBACK)
        function queueAndPlay(path, onEnded = null) {
            queue.push({ path, onEnded });
            if (!isPlaying) {
                playNext();
            }
        }
        ...
        return { queueAndPlay, stopAndClear };
    })();
    // ===================================================================
    ...
    function mostrarAlertaGanador() {
        ...
        // DETENEMOS CUALQUIER COLA ACTIVA Y ENCOLAMOS EL AUDIO DEL GANADOR
        AudioPlayer.queueAndPlay(`/VOZ DE BINGO/GANADOR/${columnNumber}.mp3`);
    ...
    function playBingoAudio(number) {
        ...
        // ENCOLAMOS el audio de la bola individual
        AudioPlayer.queueAndPlay(audioPath);
    }
    ...
    function actualizar(estado) {
        ...
        // --- INTERRUPTOR DE SONIDO (BOLA INDIVIDUAL) ---
    ...
    return { 
        ...
        actualizar, 
        renderizarHistorial, 
        AudioPlayer, // Exportado para Bienvenida y Despedida
        elementos,
    };
})();